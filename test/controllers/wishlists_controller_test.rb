...
class WishlistsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create wishlist" do
    assert_difference("Wishlist.count") do
      post wishlists_url, params: { wishlist: { name: @wishlist.name, status: @wishlist.status } }
    end
  ...
  test "should update wishlist" do
    patch wishlist_url(@wishlist), params: { wishlist: { name: @wishlist.name, status: @wishlist.status } }
