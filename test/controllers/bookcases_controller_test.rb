...
class BookcasesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create bookcase" do
    assert_difference("Bookcase.count") do
      post bookcases_url, params: { bookcase: { name: @bookcase.name, status: @bookcase.status, virtual: @bookcase.virtual } }
    end
  ...
  test "should update bookcase" do
    patch bookcase_url(@bookcase), params: { bookcase: { name: @bookcase.name, status: @bookcase.status, virtual: @bookcase.virtual } }
